...
let get_constructors (consmap : (variant * datatype) Name_map.t) typ =
  ...
  | Alias (_, Variants (id, l)) as dt -> List.fold_left (fold dt) consmap l
  | _ -> consmap
;;
...
let rec walk_type_bindings symtab (r, l) =
  let make_pending ((_, name, _) : Ast.type_binding) = (name, Pending name) in
  let make_alias (name, typ) = (name, Alias (name, typ)) in
  let from_binding symtab ((_, name, _) as binding : Ast.type_binding) =
    (* let () = print_endline ("from binding " ^ name) in *)
    (name, walk_type_binding symtab binding)
  in
  let add_binding symtab (name, typ) = add_type name typ symtab in
  let update_binding symtab (name, typ) = (name, update_pending symtab typ) in
  let resolve_binding symtab = function
    | _, Alias (_, typ) -> resolve_pending symtab typ
    | _, typ -> resolve_pending symtab typ
  in
  let filter_binding (_, typ) = filter_pending typ in
  (* let print (_, typ) =
    ...
    if r then
      ...
      let rec iter symtab completes pendings = function
        ...
        | n ->
            ...
            if List.length pendings = 0 then (symtab, completes, pendings)
            else
      ...
      if List.length pendings > 0 then
        ...
        (symtab, completes) (* non-recursive definitions *)
    else
...
and walk_type_binding symtab ((params_opt, name, construct) : Ast.type_binding)
  ...
  | None -> walk_type_construct symtab tvtab construct
  | Some tyvars ->
      let tyvars = List.map (fun x -> new_tyvar x) tyvars in
      let update_tvtab tvtab (name, id) = bind_tyvar name id tvtab in
...
and walk_type_op symtab tvtab is_decl =
  ...
  | Ast.SingleType expr -> (
      match expr with
      | Ast.TypeName name -> (tvtab, (lookup_type name symtab : datatype))
      | Ast.TypeSymbol name -> (
          if is_decl then (tvtab, Tyvar (name, lookup_tyvar name tvtab))
          else
...
and walk_type_expr symtab tvtab t =
  let _, typ = walk_type_op symtab tvtab true t in
  typ

and walk_type_annotation symtab tvtab t = walk_type_op symtab tvtab false t
...
let rec agree_on_type tvtab dest src =
  ...
  | Tyvar (_, id1), Tyvar (_, id2) ->
      ...
      let tvtab =
        match (t1, t2) with
        | Tyvar (_, root1), Tyvar (_, root2) when root1 = root2 -> tvtab
        | t1, Tyvar (_, root2) -> add_id root2 t1 tvtab
        | Tyvar (_, root1), t2 -> add_id root1 t2 tvtab
        | t1, t2 -> agree_on_type tvtab t1 t2
      in
...
and agree_on_specifics tvtab dest src =
  ...
  | ( Specific (dargs, Alias (dname, Generics (did, _, _)))
    , Specific (sargs, Alias (sname, Generics (sid, _, _))) ) ->
      if did <> sid then raise (Incompatible_types (dname, sname))
...
let make_tyvars (tyvars : tyvar list) =
  let idmapping =
    List.map (fun (name, id) -> (id, Tyvar (new_tyvar name))) tyvars
  in
...
let rec walk_value_bindings symtab tvtab (r, l) =
  if r then
    ...
    let init_bindings
      ...
      , tvtab
      , typ :: pattern_typs
      , bindings @ acc_bindings
      , ast :: acc_asts
      , names :: acc_names )
    in
    let symtab, tvtab, pattern_typs, bindings, pattern_asts, acc_names =
      List.fold_left init_bindings (symtab, tvtab, [], [], [], []) l
    in
    ...
    in
    let tvtab, expr_asts =
      List.fold_left fill_bindings (tvtab, []) (List.combine pattern_typs l)
    in
    let expr_asts = List.rev expr_asts in
    let binding_asts = combine3 acc_names pattern_asts expr_asts in
    let ast =
      Typed_ast.update_typed_ast tvtab
        (Typed_ast.ValueBindings (r, binding_asts))
    in
    let bindings =
      List.map (fun (k, v) -> (k, apply_real_type tvtab v)) bindings
    in
    let symtab = List.fold_left fold_symtab symtab bindings in
    (symtab, tvtab, ast)
  else
    ...
    let walk_value_binding (pattern, expr) =
      let bindings, tvtab, pattern_typ, pattern_ast =
        walk_pattern symtab tvtab pattern
      in
      let names, _ = List.split bindings in
      let tvtab, expr_typ, expr_ast = walk_expr symtab tvtab expr in
      let tvtab = agree_on_type tvtab pattern_typ expr_typ in
    ...
    in
    let bindings_list, binding_asts =
      List.split (List.map walk_value_binding l)
    in
...
and walk_variable symtab tvtab = function
  ...
  | _ -> failwith "walk_variable: not implemented"
...
and walk_binary symtab tvtab (op, a, b) =
  ...
  | Ast.Cons ->
      ...
      let ast =
        Typed_ast.Construct
          ( Types.list_cons
          , Typed_ast.Tuple ([ a_ast; b_ast ], Types.Tuple [ item_t; list_typ ])
          , list_typ )
      in
      (tvtab, list_typ, ast)
  | Ast.Append ->
      let list_typ =
        Specific ([ Tyvar (new_tyvar "a") ], lookup_type "list" symtab)
      in
...
and walk_if_expr symtab tvtab (cond, then_e, else_opt) =
  ...
  | Some else_e ->
      ...
      ( tvtab
      , then_t
      , Typed_ast.IfExpr (cond_ast, then_ast, Some else_ast, then_t) )
...
and walk_match_expr symtab tvtab (e, l) =
  ...
  let aux (tvtab, branch_asts) (pattern, body) =
    let bindings, tvtab, pattern, pattern_ast =
      walk_pattern symtab tvtab pattern
    in
    let varnames = List.map fst bindings in
    let tvtab = agree_on_type tvtab acc_pattern pattern in
    let () =
      debug_value2 "[branch-pattern]" pattern (apply_real_type tvtab pattern)
    in
    let bindings =
      List.map (fun (k, v) -> (k, apply_real_type tvtab v)) bindings
    in
...
and walk_pattern symtab tvtab = function
  ...
  | Ast.TuplePattern l ->
      ...
      in
      let tvtab, bindings, typs, asts =
        List.fold_left aux (tvtab, [], [], []) l
      in
      let bindings = List.rev bindings in
      let typ = Tuple (List.rev typs) in
      (bindings, tvtab, typ, Typed_ast.TuplePattern (List.rev asts, typ))
  | Ast.ConsPattern (hd, tl) ->
      walk_variant_pattern symtab tvtab
        ("(::)", Some (Ast.TuplePattern [ hd; tl ]))
  | Ast.ListPattern l ->
  ...
  | Ast.VariantPattern (name, p) -> walk_variant_pattern symtab tvtab (name, p)
  | Ast.PatternList l -> walk_pattern_list symtab tvtab l
...
and walk_variant_pattern symtab tvtab (name, pattern_opt) =
  ...
  | Some p ->
      ...
      , tvtab
      , typ
      , Typed_ast.VariantPattern (variant, Some pattern_ast, typ) )
...
and walk_pattern_list symtab tvtab l =
  ...
  in
  let bindings_list, tvtab, pattern_asts =
    List.fold_left aux ([], tvtab, []) l
  in
  let compare (name1, _) (name2, _) = compare name1 name2 in
    ...
    | bindings :: l ->
        ...
        let rec combine tvtab acc = function
          ...
          | ((acc_name, acc_typ), (binding_name, binding_typ)) :: l ->
              if acc_name <> binding_name then raise Type_mismatch
              else
