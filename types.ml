...
(* an incomplete type with only a name *)
...
let new_tyvar name = (name, new_tyvar_id ())
...
let lookup_constructor name { constructors } =
  match Name_map.find_opt name constructors with
  | None -> raise (Unbound_constructor name)
  | Some con -> con
;;

let add_constructor name con ({ constructors } as symtab) =
...
let empty_tvtab = { tvmap = Name_map.empty; idmap = Id_map.empty }
...
let lookup_tyvar_opt name { tvmap } = Name_map.find_opt name tvmap
...
let rec find_id key tvtab =
  ...
  | None ->
      (* found nothing *)
      let tvtab = add_id key tyvar tvtab in
      (* update *)
      (tvtab, tyvar)
  | Some (Tyvar (_, id) as t) when id = key ->
      (* found itself *)
      (tvtab, t)
  | Some (Tyvar (_, id)) ->
      (* found other type variable *)
      let tvtab, t = find_id id tvtab in
      (* find root *)
      let tvtab = add_id key t tvtab in
      (* update the path to root *)
      (tvtab, t)
  | Some t ->
      (* found a datatype other than type variable *)
      (tvtab, t)
...
and lookup_id key { idmap } = Id_map.find_opt key idmap

(* Stringify *)
let rec string_of_toplevel_datatype = function
  | Alias (name, Generics (_, tyvars, typ)) -> (
      let fold tvtab (name, id) = bind_tyvar name id tvtab in
...
and enclose p1 p2 name = if p1 < p2 then "(" ^ name ^ ")" else name
...
and string_of_datatype tvtab priority = function
  ...
  | Generics (id, params, body) -> (tvtab, "generics: " ^ string_of_int id)
  | Specific (args, body) ->
      let rec iter tvtab acc = function
        | [] -> (tvtab, acc)
        | arg :: l ->
...
let print_type name typ = print_endline (string_of_toplevel_datatype typ)
...
let rec update_pending symtab = function
  ...
  | Variants _ as t -> t
  | Function (arg, body) ->
      Function (update_pending symtab arg, update_pending symtab body)
  | Generics (id, params, body) ->
      Generics (id, params, update_pending symtab body)
  | Specific (args, body) ->
...
(* Resolve pending types inside variants *)
let rec resolve_pending symtab = function
  | Unit | Bool | Int | Char | String | Ref | Tyvar _ | Alias _ -> ()
  ...
  | Function (arg, body) ->
      resolve_pending symtab arg;
      resolve_pending symtab body
  | Generics (_, _, body) -> resolve_pending symtab body
  | Specific (args, body) ->
      List.iter (resolve_pending symtab) args;
      resolve_pending symtab body
...
let rec filter_pending = function
  | Unit | Bool | Int | Char | String | Ref | Tyvar _ -> false
  | Tuple l ->
...
let extract_alias = function Alias (_, typ) -> typ | t -> t

let extract_generics_id (id, params, typ) =
  (* TODO: wrong here !! *)
  match typ with
  | Specific (_, Alias (_, Generics (inner_id, vars, _)))
    when List.length params = List.length vars ->
      inner_id
  | _ -> id
;;
...
let rec check_datatype = function
  ...
  | Specific (args, body) -> (
      List.iter check_datatype args;
      match body with
      | Alias (name, Generics (_, tyvars, _)) ->
          if List.length args <> List.length tyvars then
            raise (Type_arity_mismatch name)
      | Alias (name, _) -> raise (Not_generic_type name)
      | t -> raise (Not_generic_type (get_type_string t)))
...
let list_type =
  ...
  let alias =
    Alias ("list", Generics (new_tyvar_id (), [ tyvar_for_list ], variants))
  in
  let tyvar = Tyvar tyvar_for_list in
...
let default_symtab =
  ...
  let default_constructor_list =
    [ ("[]", (list_nil, list_type)); ("(::)", (list_cons, list_type)) ]
  in
...
let rec apply_real_type tvtab = function
  ...
  | Tuple l -> Tuple (List.map (apply_real_type tvtab) l)
  | Variants _ as t -> t
  | Function (arg, body) ->
  ...
  | Specific (args, body) ->
      ...
      Specific (args, body)
  | Alias (name, typ) -> Alias (name, apply_real_type tvtab typ)
  | t ->
...
let rec apply_generics tvtab = function
  | (Unit | Bool | Int | Char | String | Ref) as t -> t
  | Tyvar (_, key) as typ -> (
      match lookup_id key tvtab with Some t -> t | None -> typ)
  | Tuple l -> Tuple (List.map (apply_generics tvtab) l)
  | Variants _ as t -> t
  | Function (arg, body) ->
