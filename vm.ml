...
let text_nameid (id, name) = string_of_int id ^ ", " ^ name
...
let get_nameid m name = Name_map.find name m

let to_map l = Name_map.of_seq (List.to_seq l)

type label = string

type op_single = Ret | Nop | Crash

let cmd_single =
  let mapping = [ ("ret", Ret); ("nop", Nop); ("crash", Crash) ] in
  get_nameid (to_map mapping)
;;

let text_single = function Ret -> "ret" | Nop -> "nop" | Crash -> "crash"

type op_jmp = JmpTrue | JmpFalse | Jmp

let cmd_jmp =
  let mapping =
    [ ("jmptrue", JmpTrue); ("jmpfalse", JmpFalse); ("jmp", Jmp) ]
  in
  get_nameid (to_map mapping)
;;
...
type op_decl = MakeFunc

let cmd_decl =
  let mapping = [ ("makefunc", MakeFunc) ] in
  get_nameid (to_map mapping)
;;

let text_decl = function MakeFunc -> "makefunc"
...
let cmd_i =
  let mapping = [ ("setnum", SetNum) ] in
  get_nameid (to_map mapping)
;;

let text_i = function SetNum -> "setnum"

type op_vb = LoadBool

let cmd_vb =
  let mapping = [ ("loadbool", LoadBool) ] in
  get_nameid (to_map mapping)
;;

let text_vb = function LoadBool -> "loadbool"

type op_vi = LoadInt

let cmd_vi =
  let mapping = [ ("loadint", LoadInt) ] in
  get_nameid (to_map mapping)
;;

let text_vi = function LoadInt -> "loadint"

type op_vc = LoadChar

let cmd_vc =
  let mapping = [ ("loadchar", LoadChar) ] in
  get_nameid (to_map mapping)
;;

let text_vc = function LoadChar -> "loadchar"

type op_vs = LoadStr

let cmd_vs =
  let mapping = [ ("loadstr", LoadStr) ] in
  get_nameid (to_map mapping)
;;

let text_vs = function LoadStr -> "loadstr"
...
and target = { label : label; mutable pos : pos }

let make_target label = { label; pos = [] }
...
let text_ssa = function
  | L l -> l
  | S op -> "  " ^ text_single op
  | J (op, target) -> "  " ^ text_jmp op ^ " " ^ target.label
  | D (op, nameid, v) ->
      "  " ^ text_decl op ^ " " ^ text_nameid nameid ^ " " ^ text_var v
  | V (op, v) -> "  " ^ text_v op ^ " " ^ text_var v
  | I (op, i) -> "  " ^ text_i op ^ " " ^ string_of_int i
  | VB (op, v, b) ->
  ...
  | VS (op, v, s) -> "  " ^ text_vs op ^ " " ^ text_var v ^ " \"" ^ s ^ "\""
  | VV (op, va, vb) -> "  " ^ text_vv op ^ " " ^ text_var va ^ " " ^ text_var vb
  | VVV (op, dest, va, vb) ->
      "  " ^ text_vvv op ^ " " ^ text_var dest ^ " " ^ text_var va ^ " "
      ^ text_var vb
  | VVI (op, dest, v, i) ->
      "  " ^ text_vvi op ^ " " ^ text_var dest ^ " " ^ text_var v ^ " "
      ^ string_of_int i
;;
...
type func =
  { id : nameid
  ; captured : nameid array (* ; prototype : unit *)
  ; nvars : int
  ; proc : proc
  }
...
let text_func func =
  ...
  let captured =
    match Array.length func.captured with
    | 0 -> ""
    | _ ->
...
let text_program program =
  let indent x = "  " ^ text_nameid x ^ "\n" in
  let exts =
    if Array.length program.exts = 0 then ""
    else
      let text = Array.fold_left ( ^ ) "" (Array.map indent program.exts) in
      ".external\n" ^ text ^ "\n"
  in
  let globs =
    if Array.length program.globs = 0 then ""
    else
...
let link_labels func =
  let rec collect m = function
    | [] -> m
    | ssa :: l as pos ->
        let m = match ssa with L label -> add_label m label pos | _ -> m in
        collect m l
  in
  ...
  let rec resolve = function
    ...
    | ssa :: l ->
        let () =
          match ssa with J (_, target) -> resolve_target map target | _ -> ()
        in
        resolve l
  in
  resolve func.proc
...
let new_frame (func, arg, captured) =
  ...
  if captnum <> capnum then raise (Captured_number_mismatch (captnum, capnum));
  { func; pos; arg; ret = Nil; vars = Array.make nvars Nil; capts = captured }
;;
...
let next_pos pos =
  match pos with [] -> failwith "next_ssa" | _ :: next -> next
;;

let next_ssa m = m.pc <- next_pos m.pc
...
let call_builtin m (id, name) =
  ...
  | 0 ->
      ...
      m.ret <- Unit
  | 1 ->
      let i = get_int m.arg in
      print_int i;
      m.ret <- Unit
  | 2 ->
      let c = get_char m.arg in
      print_char c;
      m.ret <- Unit
  | 3 ->
...
and run_j m (op, target) =
  match op with
  | JmpTrue -> if m.cond then m.pc <- target.pos else next_ssa m
  | JmpFalse -> if not m.cond then m.pc <- target.pos else next_ssa m
  | Jmp -> m.pc <- target.pos
...
and run_v m (op, var) =
  ...
  | GetRet ->
      ...
      next_ssa m
  | GetArg ->
      set_var m m.frame.arg var;
      next_ssa m
  | SetRet ->
      m.frame.ret <- get_var m var;
      next_ssa m
  | Call -> (
...
and run_vb m (op, var, b) =
  (match op with LoadBool -> set_var m (Bool b) var);
  next_ssa m

and run_vi m (op, var, i) =
  (match op with LoadInt -> set_var m (Int i) var);
  next_ssa m

and run_vc m (op, var, c) =
  (match op with LoadChar -> set_var m (Char c) var);
  next_ssa m

and run_vs m (op, var, s) =
  (match op with LoadStr -> set_var m (String s) var);
  next_ssa m
