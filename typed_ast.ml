...
let rec update_typed_ast tvtab = function
  | ValueBindings bindings ->
    update_value_bindings tvtab bindings
...
and update_pattern tvtab = function
  ...
  | ConsPattern (a, b, t) ->
    ConsPattern (update_pattern tvtab a, update_pattern tvtab b, apply_real_type tvtab t)
  | ArrayPattern (l, t) ->
    ArrayPattern (List.map (update_pattern tvtab) l, apply_real_type tvtab t)
  | RecordPattern (l, t) ->
    failwith "update_pattern"
  | VariablePattern (name, t) ->
    VariablePattern (name, apply_real_type tvtab t)
  | RefPattern (v, t) ->
    RefPattern (update_pattern tvtab v, apply_real_type tvtab t)
  | Wildcard t ->
    Wildcard (apply_real_type tvtab t)
...
and update_expr tvtab = function
  ...
  | Construct (v, e, t) -> Construct (v, update_expr tvtab e, apply_real_type tvtab t)
  ...
  | List (l, t) ->
    List (List.map (update_expr tvtab) l, apply_real_type tvtab t)
  | Array (l, t) ->
    Array (List.map (update_expr tvtab) l, apply_real_type tvtab t)
  | Record (l, t) ->
    failwith "update_expr"
  | Call (clr, cle, t) ->
    Call (update_expr tvtab clr, update_expr tvtab cle, apply_real_type tvtab t)
  | Unary (op, v, t) ->
    Unary (op, update_expr tvtab v, apply_real_type tvtab t)
  | Binary (op, a, b, t) ->
    Binary (op, update_expr tvtab a, update_expr tvtab b, apply_real_type tvtab t)
  | Local (b, e, t) ->
    Local (update_value_bindings tvtab b, update_expr tvtab e, apply_real_type tvtab t)
  | IfExpr e -> update_if_expr tvtab e
  | MatchExpr e -> update_match_expr tvtab e
  | LambdaExpr (names, p, e, t) ->
    LambdaExpr (names, update_pattern tvtab p,
      update_expr tvtab e, apply_real_type tvtab t)
  | ExprList (l, t) ->
...
and update_variable tvtab = function
  | (None, name, typ) ->
    Variable (None, name, apply_real_type tvtab typ)
  | (Some e, name, typ) ->
    Variable (Some (update_expr tvtab e), name, apply_real_type tvtab typ)

and update_if_expr tvtab = function
  | (cond, then_, None, typ) ->
    IfExpr (update_expr tvtab cond, update_expr tvtab then_,
      None, apply_real_type tvtab typ)
  | (cond, then_, Some else_, typ) ->
    IfExpr (update_expr tvtab cond, update_expr tvtab then_,
      Some (update_expr tvtab else_), apply_real_type tvtab typ)
