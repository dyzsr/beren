...
let read_token_until_eof lexer lexbuf =
  let rec aux = function Parser.EOF -> () | v -> aux (lexer lexbuf) in
  aux (lexer lexbuf)
;;
...
let split_extension filename =
  ...
  | ext :: names ->
      if ext <> "brn" then raise (Invalid_filename filename)
      else
...
let _ =
  let _ =
    match Array.length Sys.argv with 1 -> failwith "no input file" | n -> n
  in
  ...
  let print_ast decl =
    print_endline (Ast.rep_to_string (Ast.decl_to_rep decl))
  in
